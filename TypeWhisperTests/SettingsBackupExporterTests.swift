...
final class SettingsBackupExporterTests: XCTestCase {
    ...
    private func makeFixture() throws -> Fixture {
        ...
        return Fixture(
            ...
            meetingService: MeetingService(appSupportDirectory: dir),
            participantDirectoryService: ParticipantDirectoryService(appSupportDirectory: dir),
            meetingContextRuleService: MeetingContextRuleService(appSupportDirectory: dir),
            userDefaults: userDefaults,
            suiteName: suiteName
        )
    }
    ...
    // MARK: - Meetings round-trip (fork adaptation of #932)

    func testRoundTripMeetingWithParticipantsAndRules() async throws {
        let source = try makeFixture()
        defer { teardown(source) }

        // A fully-populated meeting: attendees, a transcript segment, a note, a generated output, a
        // Q&A turn, plus folder / tags / language / two-person metadata.
        let attendees = [
            Attendee(name: "Ada Lovelace", email: "ada@example.com"),
            Attendee(name: "Alan Turing", email: "alan@example.com")
        ]
        let meeting = source.meetingService.createMeeting(
            title: "Quarterly Planning",
            source: .adHoc,
            state: .completed,
            attendees: attendees
        )
        source.meetingService.appendStableSegments(
            [TranscriptionSegment(text: "Welcome everyone.", start: 0, end: 2.5, speakerLabel: "SPEAKER_00", speakerConfidence: 0.9)],
            source: .liveCapture,
            to: meeting
        )
        _ = source.meetingService.addNote(to: meeting, text: "Discuss roadmap", timestampOffset: 1.0)
        _ = source.meetingService.addOutput(to: meeting, kind: .summary, content: "A concise summary.", providerUsed: "openai", modelUsed: "gpt-4o")
        _ = source.meetingService.addQATurn(to: meeting, question: "What's the deadline?", answer: "End of Q3.")
        source.meetingService.setFolder("Work/Planning", for: meeting)
        source.meetingService.setObsidianTags(["planning", "q3"], for: meeting)
        source.meetingService.setLanguage("en", for: meeting)
        source.meetingService.setTwoPersonCall(true, for: meeting)

        // Participants directory + a capture-context rule.
        source.participantDirectoryService.ingest(attendees)
        XCTAssertGreaterThan(source.participantDirectoryService.persons.count, 0)
        _ = source.meetingContextRuleService.createRule(
            name: "Client calls",
            trigger: MeetingRuleTrigger(attendeeDomains: ["example.com"], titleKeywords: ["planning"]),
            actions: MeetingRuleActions(liveEngineId: "whisperkit", languageSelection: "en"),
            isEnabled: true
        )

        // A portable meetings preference must survive too.
        source.userDefaults.set(true, forKey: UserDefaultsKeys.meetingsPreferProviderSpeakerLabels)
        source.userDefaults.set("Meetings", forKey: UserDefaultsKeys.meetingsObsidianRootFolder)
        ...
        let backup = SettingsBackupExporter.buildBackup(
            ...
            pluginManager: source.pluginManager,
            historyService: source.historyService,
            meetingService: source.meetingService,
            participantDirectoryService: source.participantDirectoryService,
            meetingContextRuleService: source.meetingContextRuleService,
            userDefaults: source.userDefaults
        )

        XCTAssertEqual(backup.meetings.count, 1)
        XCTAssertEqual(backup.participants.count, source.participantDirectoryService.persons.count)
        XCTAssertEqual(backup.meetingRules.count, 1)

        // Round-trip through the on-disk JSON representation.
        let data = try SettingsBackupExporter.encodedJSON(backup)
        let parsed = try SettingsBackupExporter.parse(data)
        ...
        let result = await SettingsBackupExporter.importBackup(
            ...
            participantDirectoryService: destination.participantDirectoryService,
            meetingContextRuleService: destination.meetingContextRuleService,
            userDefaults: destination.userDefaults
        )

        XCTAssertEqual(result.meetingsImported, 1)
        XCTAssertEqual(result.participantsImported, source.participantDirectoryService.persons.count)
        XCTAssertEqual(result.meetingRulesImported, 1)
        XCTAssertGreaterThan(result.meetingPreferencesApplied, 0)

        // Meeting aggregate restored faithfully.
        let imported = try XCTUnwrap(destination.meetingService.meetings.first)
        XCTAssertEqual(destination.meetingService.meetings.count, 1)
        XCTAssertEqual(imported.id, meeting.id)
        XCTAssertEqual(imported.title, "Quarterly Planning")
        XCTAssertEqual(imported.state, .completed)
        XCTAssertEqual(imported.languageCode, "en")
        XCTAssertEqual(imported.folderPath, "Work/Planning")
        XCTAssertEqual(Set(imported.tags), ["planning", "q3"])
        XCTAssertEqual(imported.twoPersonCall, true)
        XCTAssertEqual(imported.attendees.map(\.email).compactMap { $0 }.sorted(), ["ada@example.com", "alan@example.com"])
        XCTAssertEqual(imported.segments.count, 1)
        XCTAssertEqual(imported.segments.first?.text, "Welcome everyone.")
        XCTAssertEqual(imported.segments.first?.speakerLabel, "SPEAKER_00")
        XCTAssertEqual(imported.notes.count, 1)
        XCTAssertEqual(imported.notes.first?.text, "Discuss roadmap")
        XCTAssertEqual(imported.outputs.count, 1)
        XCTAssertEqual(imported.outputs.first?.kind, .summary)
        XCTAssertEqual(imported.outputs.first?.content, "A concise summary.")
        XCTAssertEqual(imported.qaTurns.count, 1)
        XCTAssertEqual(imported.qaTurns.first?.answer, "End of Q3.")

        // Participants restored.
        XCTAssertEqual(destination.participantDirectoryService.persons.count, source.participantDirectoryService.persons.count)
        XCTAssertTrue(destination.participantDirectoryService.persons.contains { $0.emailKey == "ada@example.com" })

        // Context rule restored with its encoded trigger/actions intact.
        let rule = try XCTUnwrap(destination.meetingContextRuleService.rules.first)
        XCTAssertEqual(destination.meetingContextRuleService.rules.count, 1)
        XCTAssertEqual(rule.name, "Client calls")
        XCTAssertTrue(rule.isEnabled)
        XCTAssertEqual(rule.trigger.attendeeDomains, ["example.com"])
        XCTAssertEqual(rule.trigger.titleKeywords, ["planning"])
        XCTAssertEqual(rule.actions.liveEngineId, "whisperkit")
        XCTAssertEqual(rule.actions.languageSelection, "en")

        // Portable meetings preference migrated.
        XCTAssertEqual(destination.userDefaults.object(forKey: UserDefaultsKeys.meetingsPreferProviderSpeakerLabels) as? Bool, true)
        XCTAssertEqual(destination.userDefaults.string(forKey: UserDefaultsKeys.meetingsObsidianRootFolder), "Meetings")
    }

    func testMeetingsCategoryExcludedWhenNotSelected() async throws {
        let source = try makeFixture()
        defer { teardown(source) }

        _ = source.meetingService.createMeeting(title: "Standup", source: .adHoc)
        source.participantDirectoryService.ingest([Attendee(name: "Grace Hopper", email: "grace@example.com")])
        _ = source.meetingContextRuleService.createRule(name: "Rule")
        ...
        let withoutMeetings = SettingsBackupExporter.filtered(backup, to: [.workflows])
        XCTAssertEqual(withoutMeetings.meetings.count, 0)
        XCTAssertEqual(withoutMeetings.participants.count, 0)
        XCTAssertEqual(withoutMeetings.meetingRules.count, 0)
        XCTAssertEqual(SettingsBackupExporter.Category.count(.meetings, in: withoutMeetings), 0)
    }
}
