...
enum SettingsBackupExporter {
    ...
    struct PreferencesDTO: Codable {
        ...
        // NOTE: `requireSecondEscapeToCancelRecording` (upstream #883/#06d6545) is intentionally not
        // backed up — that recording-cancel-confirmation feature is deferred in this fork, so its
        // UserDefaults key does not exist here.
        // Dictation Recovery
    ...
    struct SettingsBackup: Codable {
        ...
        // Meetings coverage (fork adaptation of #932). Declared with defaults so the synthesized
        // memberwise initializer stays source-compatible with call sites (and fixtures) that predate
        // meetings support; a real export always populates them.
        var meetings: [MeetingDTO] = []
        var participants: [PersonDTO] = []
        var meetingRules: [MeetingContextRuleDTO] = []
        var meetingPreferences: MeetingPreferencesDTO = .empty
    }
    ...
    struct ImportResult {
        ...
        var participantsImported = 0
        var meetingRulesImported = 0
        var meetingPreferencesApplied = 0
    }
    ...
    enum Category: String, CaseIterable, Identifiable, Hashable {
        case workflows, dictionary, snippets, profiles, promptActions, hotkeys, plugins, history, meetings, preferences
        ...
        var title: String {
            ...
            case .meetings: return String(localized: "Meetings")
        ...
        var infoText: String {
            ...
            case .meetings: return localizedAppText(
                    "Meetings with their transcripts, notes, generated outputs, participants, and capture-context rules, plus portable meetings preferences. Saved meeting audio is never included.",
                    de: "Meetings mit Transkripten, Notizen, generierten Ausgaben, Teilnehmern und Aufnahme-Kontextregeln sowie portable Meeting-Einstellungen. Gespeicherte Meeting-Audioaufnahmen sind nie enthalten."
                )
        ...
        static func count(_ category: Category, in backup: SettingsBackup) -> Int {
            ...
            case .meetings:
                return backup.meetings.count
                    + backup.participants.count
                    + backup.meetingRules.count
                    + backup.meetingPreferences.nonNilCount
    ...
    static func filtered(_ backup: SettingsBackup, to categories: Set<Category>) -> SettingsBackup {
        ...
        return SettingsBackup(
            ...
            meetings: categories.contains(.meetings) ? backup.meetings : [],
            participants: categories.contains(.meetings) ? backup.participants : [],
            meetingRules: categories.contains(.meetings) ? backup.meetingRules : [],
            meetingPreferences: categories.contains(.meetings) ? backup.meetingPreferences : .empty
        )
    }
    ...
    static func buildBackup(
        ...
        participantDirectoryService: ParticipantDirectoryService? = nil,
        meetingContextRuleService: MeetingContextRuleService? = nil,
        ...
        return SettingsBackup(
            ...
            meetings: meetingService.map { meetingDTOs(from: $0.meetings) } ?? [],
            participants: participantDirectoryService.map { personDTOs(from: $0.persons) } ?? [],
            meetingRules: meetingContextRuleService.map { meetingRuleDTOs(from: $0.rules) } ?? [],
            meetingPreferences: meetingService == nil ? .empty : meetingPreferences(from: userDefaults)
        )
    }
    ...
    static func importBackup(
        ...
        // Meetings restore (fork adaptation of #932). Participants and rules are restored before
        // meetings so a meeting's attendees can resolve against the freshly-imported directory.
        // Every path is additive-by-id inside its single-writer service.
        if let participantDirectoryService {
            result.participantsImported = participantDirectoryService.importPersons(backup.participants)
        }
        if let meetingContextRuleService {
            result.meetingRulesImported = meetingContextRuleService.importRules(backup.meetingRules)
        }
        if let meetingService {
            result.meetingsImported = meetingService.importMeetings(backup.meetings)
            result.meetingPreferencesApplied = applyMeetingPreferences(backup.meetingPreferences, to: userDefaults)
        }
        ...
        for (index, entry) in backup.history.enumerated() {
            ...
            // visible in History.
            if inserted {
                // Our `UsageStatisticsService` (adopted from #839) records aggregate word/app/
                // time-saved stats only; the richer per-engine/-app fields #932 was written against
                // (upstream's later Statistics-tab expansion) aren't part of this fork's schema.
